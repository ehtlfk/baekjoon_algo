...
# 두가지 놓친점
# 1. 우선 stack_l과 stack_r이 원본과 합쳐지면서 생길 수 있는 경우의 수
# 2. 그리고 만약 원본의 마지막이 키워드여서 종료될 경우, 원본은 비어있음. 그 상태에서 left와 right가 결합해서
# 생성될 수 있는 경우( 대부분의 문제에서 마지막에서 사라질 경우 원본이 비어있으므로 발생하는 문제, 항상 마지막을 생각해라!)